...
def unet3D(input_img, use_upsampling=False, n_out=1, dropout=0.2,
    ...
    conv1 = K.layers.Conv3D(name="conv1a", filters=args.fms, **params)(inputs)
    conv1 = K.layers.BatchNormalization()(conv1)
    conv1 = K.layers.Activation("relu")(conv1)
    conv1 = K.layers.Conv3D(name="conv1b", filters=args.fms*2, **params)(conv1)
    conv1 = K.layers.BatchNormalization()(conv1)
    conv1 = K.layers.Activation("relu")(conv1)
    pool1 = K.layers.MaxPooling3D(name="pool1", pool_size=(2, 2, 2))(conv1)

    conv2 = K.layers.Conv3D(name="conv2a", filters=args.fms*2, **params)(pool1)
    conv2 = K.layers.BatchNormalization()(conv2)
    conv2 = K.layers.Activation("relu")(conv2)
    conv2 = K.layers.Conv3D(name="conv2b", filters=args.fms*4, **params)(conv2)
    conv2 = K.layers.BatchNormalization()(conv2)
    conv2 = K.layers.Activation("relu")(conv2)
    pool2 = K.layers.MaxPooling3D(name="pool2", pool_size=(2, 2, 2))(conv2)

    conv3 = K.layers.Conv3D(name="conv3a", filters=args.fms*4, **params)(pool2)
    ...
    conv3 = K.layers.Conv3D(name="conv3b", filters=args.fms*8, **params)(conv3)
    conv3 = K.layers.BatchNormalization()(conv3)
    conv3 = K.layers.Activation("relu")(conv3)
    pool3 = K.layers.MaxPooling3D(name="pool3", pool_size=(2, 2, 2))(conv3)

    conv4 = K.layers.Conv3D(name="conv4a", filters=args.fms*8, **params)(pool3)
    ...
    conv4 = K.layers.Conv3D(name="conv4b", filters=args.fms*16, **params)(conv4)
    conv4 = K.layers.BatchNormalization()(conv4)
    conv4 = K.layers.Activation("relu")(conv4)
    ...
    if use_upsampling:
        ...
        up = K.layers.Conv3DTranspose(name="transConv4", filters=args.fms*16, data_format=data_format,
                           kernel_size=(2, 2, 2), strides=(2, 2, 2), padding="same")(conv4)

    up4 = K.layers.concatenate([up, conv3], axis=concat_axis)

    conv5 = K.layers.Conv3D(name="conv5a", filters=args.fms*16, **params)(up4)
    conv5 = K.layers.BatchNormalization()(conv5)
    conv5 = K.layers.Activation("relu")(conv5)
    conv5 = K.layers.Conv3D(name="conv5b", filters=args.fms*16, **params)(conv5)
    conv5 = K.layers.BatchNormalization()(conv5)
    conv5 = K.layers.Activation("relu")(conv5)
    ...
    if use_upsampling:
        ...
        up = K.layers.Conv3DTranspose(name="transConv5", filters=args.fms*8, data_format=data_format,
                           kernel_size=(2, 2, 2), strides=(2, 2, 2), padding="same")(conv5)

    up5 = K.layers.concatenate([up, conv2], axis=concat_axis)

    conv6 = K.layers.Conv3D(name="conv6a", filters=args.fms*4, **params)(up5)
    conv6 = K.layers.BatchNormalization()(conv6)
    conv6 = K.layers.Activation("relu")(conv6)
    conv6 = K.layers.Conv3D(name="conv6b", filters=args.fms*4, **params)(conv6)
    conv6 = K.layers.BatchNormalization()(conv6)
    conv6 = K.layers.Activation("relu")(conv6)
    ...
    if use_upsampling:
        ...
        up = K.layers.Conv3DTranspose(name="transConv6", filters=args.fms*4, data_format=data_format,
                           kernel_size=(2, 2, 2), strides=(2, 2, 2), padding="same")(conv6)

    up6 = K.layers.concatenate([up, conv1], axis=concat_axis)

    conv7 = K.layers.Conv3D(name="conv7a", filters=args.fms*2, **params)(up6)
    conv7 = K.layers.BatchNormalization()(conv7)
    conv7 = K.layers.Activation("relu")(conv7)
    conv7 = K.layers.Conv3D(name="conv7b", filters=args.fms*2, **params)(conv7)
    conv7 = K.layers.BatchNormalization()(conv7)
    conv7 = K.layers.Activation("relu")(conv7)
...
if args.single_class_output:
    ...
    opt = hvd.DistributedOptimizer(K.optimizers.Adam(lr=args.lr))
...
total_steps = args.num_datapoints// (args.bz*hvd.size())
